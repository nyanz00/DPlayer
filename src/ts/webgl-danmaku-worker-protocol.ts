...
export type WebGLDanmakuWorkerInput =
    | { type: 'init', canvas: OffscreenCanvas, width: number, height: number, pixelRatio: number, opacity: number }
    | { type: 'add', item: WorkerDanmakuItem, bitmap: ImageBitmap }
    | { type: 'remove', id: number }
    | { type: 'resize', width: number, height: number, pixelRatio: number }
    | { type: 'opacity', value: number }
    | { type: 'clock', mediaTime: number | null, sampledAt: number, playing: boolean, playbackRate: number }
    | { type: 'clear' };
